 ...
 * @since 1.8
 */
public final class ConvertOptional<T> {
    /**
     * Common instance for {@code empty()}.
     */
    private static final ConvertOptional<?> EMPTY = new ConvertOptional<>();
     ...
     *
     * @implNote Generally only one empty instance, {@link ConvertOptional#EMPTY},
     * should exist per VM.
     */
     ...
     */
    public static <T> ConvertOptional<T> empty() {
        @SuppressWarnings("unchecked")
        ConvertOptional<T> t = (ConvertOptional<T>) EMPTY;
        return t;
    }
     ...
     */
    private ConvertOptional(T value) {
     ...
     * @throws NullPointerException if value is {@code null}
     */
    public static <T> ConvertOptional<T> of(T value) {
        return new ConvertOptional<>(value);
    }
     ...
     */
    public static <T> ConvertOptional<T> ofNullable(T value) {
     ...
     */
    public ConvertOptional<T> filter(Predicate<? super T> predicate) {
     ...
     * URI if one exists.
     */
    public <U> ConvertOptional<U> map(Function<? super T, ? extends U> mapper) {
        Objects.requireNonNull(mapper);
        if (!isPresent()) {
            return empty();
        } else {
            return ConvertOptional.ofNullable(mapper.apply(value));
        }
    }
     ...
     */
    public <U> ConvertOptional<U> flatMap(Function<? super T, ? extends ConvertOptional<? extends U>> mapper) {
        Objects.requireNonNull(mapper);
        if (!isPresent()) {
            return empty();
        } else {
            @SuppressWarnings("unchecked")
            ConvertOptional<U> r = (ConvertOptional<U>) mapper.apply(value);
     ...
     * @since 9
     */
    public ConvertOptional<T> or(Supplier<? extends ConvertOptional<? extends T>> supplier) {
        Objects.requireNonNull(supplier);
        if (isPresent()) {
            return this;
        } else {
            @SuppressWarnings("unchecked")
            ConvertOptional<T> r = (ConvertOptional<T>) supplier.get();
            return Objects.requireNonNull(r);
        }
    }
     ...
     * <pre>{@code
     *     Stream<Optional<U>> os = ..
     *     Stream<U> s = os.flatMap(Optional::stream)
     * }</pre>
     * @since 9
     */
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ConvertOptional)) {
            return false;
        }

        ConvertOptional<?> other = (ConvertOptional<?>) obj;
    ...
    @SuppressWarnings("unchecked")
    public <U> ConvertOptional<U> convert(Class<U> convertClass) {
        return ofNullable((U) (convertClass.isAssignableFrom(this.value.getClass()) ? this.value : null));
    }
